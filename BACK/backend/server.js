...
const bodyParser = require('body-parser'); // Ajouter le body-parser
...
// Middleware
app.use(bodyParser.json()); // Utiliser bodyParser pour parser les requêtes JSON
app.use(cors({
  origin: 'http://localhost:3000', // Autorise seulement votre frontend
  methods: ['GET', 'POST', 'PUT', 'DELETE'], // Liste des méthodes autorisées
}));

// Connect to MongoDB
// MongoDB connection
mongoose.connect('mongodb+srv://walid23:123@cluster0.ubtxv.mongodb.net/VLAN')
...
function runPythonScript(action, vlanId, vlanName = '') {
  return new Promise((resolve, reject) => {
    ...
    console.log('Executing command:', command); // Add logging
